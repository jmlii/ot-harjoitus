...
Ohjelmakoodissa on käytetty vain vähän try-lausekkeita, joiden avulla virhetilanteita voisi hallita paremmin. Nyt ohjelma on kirjoitetu pääasiassa siten, että virhetilanteita ei pääse edes syntymään, joten niille ei ole luotu hallintaakaan. 

Dao-rajapinnat ja ne toteuttuvat luokat olisi voinut toteuttaa siten, että ne palauttavat sovellukselle tiedon onnistumisesta tai epäonnistumisesta esim. boolean-arvoina. Sovellus voisi hyödyntää tietoja esimerkiksi välittämällä sovelluslogiikan avulla tiedot käyttöliittymässä annettaviksi viesteiksi toimintojen onnistumisesta tai epäonnistumisesta. 

Käyttöliittymän tapahtumalomakkeet rakentava metodi on pitkä ja hieman vaikeaselkoinen monine haaroineen. Metodiin on sisällytetty kaikkien mahdollisten tapahtumalomakkeiden vaihtoehtoiset rakennusosat: komponentit tulo- ja menotapahtumille, sekä uuden tapahtuman lisäämiseen kuin aiemmin tallennetun tapahtuman muokkaamiseen. Metodi saa kutsunsa yhteydessä parametreja, joiden perusteella oikeat osat valikoituvat näytettäviksi. Metodin olisi voinut pilkkoa useammaksi pienemmäksi metodiksi, joista kutsutaan vain kulloinkin tarvittavia. 
...
Lisäksi on huomattava, että käyttäjä voi lisätä tietoja suoraan tietokantaan ilman sovellusta, joten sitä kautta käyttäjä voi luoda sellaisiakin kategorioita, joita sovellus ei luo. Käyttäjän poistaessa kategorian suoraan tietokannasta ohjelma kuitenkin luo sen uudestaan. Lisäksi tietokanta sallii muuttaa tietokannassa menokategorioita tulokategoriaksi tai luoda uusia tulokategorioita. Sovellus ei kuitenkaan huomioi tulojen lisäyksessä näitä uusia tulokategorioita, mutta menokategorioista tulokategorioiksi muutetut poistuvat menotapahtumien lomakkeen kategoriavaihtoehdoista. Tietokanta ei myöskään tarkista tapahtumien arvoja, joten tietokantaan voi tallentaa positiivisia menoarvoja ja negatiivisia tuloarvoja. Vastaavasti ohjelma ei tarkista tietokannassa olevia lukuja ja muuta "vääränmuotoisia" arvoja "oikeanmuotoisiksi", joten ne sekoittavat sovelluksen käyttöä käyttöliittymässä. Käyttäjää ei ohjeisteta tietokannan suoraan käyttöön, koska sitä on tarkoituksenmaukaista käyttää vain sovelluksen kautta, jos ei tiedä mitä on tekemässä. 
